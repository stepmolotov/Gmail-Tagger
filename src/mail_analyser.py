...
class MailAnalyser:
    ...
    def __init__(self, config: Config) -> None:
        self._config = config
        marvin.settings.openai.api_key = self._config.oa_key
    ...
    def get_details(self, email: Email) -> EmailDetails:
        ...
        email_details: list[EmailDetails] = marvin.extract(
            raw_input,
            EmailDetails,
            instructions="Given an email subject, snippet and text, get the top 5 keywords from the email text and identify sender and topic.",
        )
        if email_details:
